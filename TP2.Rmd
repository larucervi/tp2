...
Para explorar dichos resultados, se formula a continuación una tabla para detectar cuáles provincias presentan los valores minimos y máximos para cada variable:
...
Para ello uno mi actual base de datos con su localización espacideal, definida por GeoAr
```{r}
arg_geo <- get_geo(geo = "ARGENTINA",
                   level="provincia",simplified = F)
```

*Reemplazar "Tierra del Fuego, Antártida e Islas del Atlántico Sur (1)" por "Tierra del Fuego" en mi columna "jurisdiccion" en mi base de datos "censo_condiciones*
```{r}
library(dplyr)

#censo_condiciones$jurisdiccion <- iconv(censo_condiciones$jurisdiccion, "UTF-8", "ASCII//TRANSLIT")
...
En primer lugar, se dispondrá la comparación provincial de tenencia propia de la vivienda y de red de gas o electricidad en pos de  una variable proxy para identificar la calidad de la vivienda. 
...
censo_condiciones %>% 
  ...
  theme_void()
```
A partir del gráfico anterior, incluso teniendo en cuenta la diversidad de la escala aplicada a cada variable, es posible ver para cada mapa la distibución de valores y su concentración. Mientras que la calidad de la vivienda merma hacia el norte, es allí también donde aumentan los valores respecto de tenencia de la vivienda en detrimentro del sur. 

Se observan entoonces algunos patrones espaciales. Para observar mejor la relación de este fenómeno, se desplegaran a continuación en un gráfico de matriz de correlaciones sobre las condiciones habitacionales entre sí mismas. 
...
```
En este gráfico puede verse de manera más clara, que la vivienda propia no sólo es la única variable que correlaciona de manera negativa con dos o más baños, sino que lo hace con todas las variables relacionadas a las condiciones habitacionales. Asimismo, lo hace de manera contundente. Si bien se necesita un mayor esclarecimiento de los datos del censo, se debe atender a esta relación entre la calidad de la vivienda que es propia y su evolución, muy frecuente
